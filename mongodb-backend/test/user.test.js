...
describe('Users', () => {
  ...
  // Before each test, clear the User collection using async/await
  beforeEach(async function() {
    this.timeout(5000); // Set timeout to 5000ms to avoid timeout issues
    try {
      await User.deleteMany();  // Use await for the promise to resolve
    } catch (err) {
      console.error(err);
    }
  });
  ...
  describe('/GET/:id user', () => {
    it('it should GET a user by the given id', (done) => {
      let user = new User({ name: "Jane Doe", email: "janedoe@example.com", password: "password123" });
      user.save((err, user) => {
        chai.request(app)
